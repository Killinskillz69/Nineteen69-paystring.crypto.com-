...
- [比特币和现有概念的介绍](#比特币和现有概念的介绍)
  - [历史](#历史)
  - [作为一种状态转移系统的比特币](#作为一种状态转移系统的比特币)
  - [挖矿](#挖矿)
  - [Merkle树](#Merkle树)
  - [替代的区块链应用程序](#替代的区块链应用程序)
  - [脚本](#脚本)
- [以太坊](#以太坊)
  - [哲学](#哲学)
  - [以太坊账户](#以太坊账户)
...
- [应用](#应用)
  - [通证系统](#通证系统)
  - [金融衍生品和稳定价值货币](#金融衍生品和稳定价值货币)
  - [身份和声誉系统](#身份和声誉系统)
  - [去中心化文件存储](#去中心化文件存储)
  - [去中心化自治组织](#去中心化自治组织)
  - [其他应用](#其他应用)
- [杂项和担忧](#杂项和担忧)
  - [修改的GHOST实现](#修改的GHOST实现)
  - [费用](#费用)
  - [计算和图灵完备性](#计算和图灵完备性)
  - [货币和发行](#货币和发行)
  - [挖矿中心化](#挖矿中心化)
...
为了更好地理解挖矿的目的，让我们看看在一个恶意攻击者的事件中会发生什么。由于比特币的底层加密被认为是安全的，因此攻击者将针把未直接受加密保护的比特币系统的一部分一部分作为目标：交易顺序。攻击者的策略很简单：
...
* **Colored coins** - [彩色硬币](https://docs.google.com/a/buterin.com/document/d/1AnkP_cVZTCMLIzw4DvsW6M8Q2JC0lIzrTLuoWu2z1BE/edit)的目的是作为一种协议，允许人们创建自己的数字货币 - 或者，在比特币区块链的一个单位货币的重要的琐碎的情况下，数字通证。在彩色硬币协议中，通过公开为特定比特币UTXO分配颜色来“发布”新货币，并且协议递归地将其他UTXO的颜色定义为与创建它们的交易的输入的颜色相同。 （一些特殊规则适用于混合颜色输入的场景）。这允许用户维护仅包含特定颜色的UTXO的钱包，并像普通比特币一样发送它们，通过区块链回溯以确定他们收到的任何UTXO的颜色。
...
5. 运行代码。在这种情况下，这很简单：它检查合约在索引`2`处的存储是否被使用，注意到它没有被使用，因此它将索引`2`处的存储设置为值`CHARLIE`。假设这需要187 gas，因此剩余的气体量为1150 - 187 = 963
...
区块链通证系统有许多应用程序，从代表资产（如美元或黄金）的子货币到公司股票，代表智能财产的个人代币（通证），安全的不可伪造的优惠券，甚至与传统价值无关的用作点数激励系统的代币（通证）系统。在以太坊中，通证系统非常容易实现。要理解的关键点是货币或通证系统从根本上说是一个具有一个操作的数据库：从A减去X单位并将X单位赋予B，条件是（1）A在交易前至少有X个单位（2）交易由A批准。实现通证系统所需的全部是将此逻辑实现到合约中。

在Serpent中实现通证系统的基本代码如下：
...
这基本上是本文件中上面进一步描述的“银行系统”状态转换功能的字面实现。需要添加一些额外的代码行来提供起初分配货币单位的初始步骤以及一些其他边缘情况，理想情况下会添加一个函数以让其他合约查询地址的余额。但这就是它的全部。从理论上讲，作为子货币的基于以太坊的通证系统可能包括基于比特币的链上元货币缺乏的另一个重要特征：直接以该货币支付交易费用的能力。在这种实现方式下，合约会维护一个ether余额，合约可以用这个ether余额来退还被用于支付交易费用的ether给发送者，并且它将通过收集收取费用并在持续的拍卖中转售它们的内部货币单位来补充这种余额。因此，用户需要用ether“激活”他们的帐户，但是一旦ether在那里它就可以重复使用，因为合同每次都会退还。
