...
Ethereum 2.0 の核であるシステムチェーンを、「ビーコンチェーン」 "beacon chain" と呼ぶ。
ビーコンチェーンは、[検証者ら](#dfn-validator)の登録、保存をする。
Ethereum 2.0 の初期配備段階において、検証者となるための唯一の方法は、Ethereum 1.0 のあるデポジットコントラクトに対し、ETH の片道トランザクションを生成することである。
検証者の活性化は、Ethereum 1.0 の預金レシートがビーコンチェーンで処理された、活性化預金残高に達し、キューにつむというプロセスがおわったときに起こります。退出は、自主的なものあるいは、誤った行動に対する罰則として強制されるかのどちらかでです。
...
数々の証明とは、ひとつのシャードブロックに対する可用性の投票（そのシャードブロックが投票者にとって利用可能な（正しい）ものであるのかの投票）であり、
...
* **Validator** <a id="dfn-validator"></a> - ビーコンチェーンに登録した参加者。Ethereum 1.0 の預金コントラクトに ETH を送ることで参加者となることができる。
...
ある committee（委員会） が、「この committee は X を証明する」といった風に、まとまって参照されたとき、これは次のことを意味するものとされる。「プロトコルが認識するのに十分な [validators](#dfn-validator) を含む、その committee のある部分集合が、その committee を代表している」
